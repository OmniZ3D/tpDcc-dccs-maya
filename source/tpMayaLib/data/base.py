...
class MayaCustomData(data.CustomData, object):
    ...
    def _center_view(self):
        ...
        try:
            ...
            maya.logger.debug('Could not center view: {}'.format(traceback.format_exc()))
...
class MayaFileData(MayaCustomData, object):
    ...
    def open(self, file_path=None):
        ...
        if not open_file:
            ...
            if not path.is_file(file_path):
                maya.logger.warning('Could not open file: {}'.format(file_path))
                return
            open_file = file_path
        ...
        try:
            ...
            maya.logger.error('Impossible to open Maya file: {} | {}'.format(open_file, traceback.format_exc()))
    ...
    def import_data(self, file_path=''):
        ...
        if not path.is_file(import_file):
            maya.logger.warning('Impossible to import invalid data file: {}'.format(file_path))
            return
    ...
    def _clean_scene(self):
        maya.logger.debug('Cleaning Maya scene ...')
...
#             current_directory = maya.cmds.file(query=True, expandName=True)
#         else:
...
#         file_path = maya.cmds.file(q=True, sn=True)
#         saved = scene.save_as(file_path)
#
#         return saved
#
...
#             maya.logger.warning('Impossible to open invalid data!')
#             return
        #
        # current_task = rigtask.get_current_task()
        # if not current_task:
        #     maya.logger.warning('Impossible to open file because task is not defined!')
        #     return

        # data_file = self._data_widget.get_file()
        # if not path.is_file(data_file):
        #     maya.logger.warning('No data to open. Please save once!')
        #     return
        #
        # if tp.Dcc.get_name() == tp.Dccs.Maya:
        #     import maya.cmds as cmds
        #     if maya.cmds.file(query=True, mf=True):
        #         file_path = maya.cmds.file(q=True, sn=True)
...
#
#         if not self._data_widget:
#             maya.logger.warning('Impossible to save invalid data!')
#             return
#
...
#     def _on_import(self):
#         if not self._data_widget:
#             maya.logger.warning('Impossible to import invalid data!')
#             return
#
...
#         if not self._data_widget:
#             maya.logger.warning('Impossible to reference invalid data!')
#             return
#
...
#     def _on_export(self):
#         if not self._data_widget:
#             maya.logger.warning('Impossible to export invalid data!')
#             return
#
#         comment = qtutils.get_comment(parent=self)
#         if comment is None:
