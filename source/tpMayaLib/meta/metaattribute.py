...
class MetaAttribute(object):
    ...
    def __init__(self, obj_name=None, attr_name=None, attr_type=False, value=None, enum=None, lock=None,
        ...
        try:
            ...
            assert cmds.objExists(obj_name) is True, '"{}" does not exists!'.format(obj_name)
            ...
            if type(value) is list:
                for o in value:
                    if cmds.objExists(o):
                        ...
                        tp.logger.debug('MultiMessage mode!')
                        break
        ...
        if cmds.objExists('{0}.{1}'.format(self.obj.meta_node, attr_name)):
            current_type = cmds.getAttr('{0}.{1}'.format(self.obj.meta_node, attr_name), type=True)
            if not metautils.MetaAttributeUtils.validate_attr_type_match(self.attr_type, current_type) and self.attr_type is not False:
                if self.obj.is_referenced():
                    tp.logger.error('"{0}" is referenced. Cannot convert "{1}" to "{2}"!'.format(self.obj.meta_node, attr_name, attr_type))
            ...
            try:
                ...
                tp.logger.error('|Attribute Add| >> Failed" "{0}" failed to add "{1}" | type: {2}'.format(self.obj.meta_node, attr_name, self.attr_type))
        ...
        if enum:
            try:
                ...
                tp.logger.error('Failed to set enum value of "{}"'.format(enum))
        ...
        if min_value is not None:
            try:
                ...
                tp.logger.error('|Attribute Add| >> min value on call failure!'.format(min_value))

        if max_value is not None:
            try:
                self.max_value = max_value
            except Exception:
                tp.logger.error('|Attribute Add| >> max value on call failure!'.format(min_value))

        if default_value is not None:
            try:
                self.default_value = default_value
            except Exception:
                tp.logger.error('|Attribute Add| >> default value on call failure!'.format(min_value))
    ...
    def get_alias(self):
        ...
        if cmds.aliasAttr(self.combined_name, query=True):
            return cmds.aliasAttr(self.combined_name, query=True)
            ...
            if alias:
                try:
                    if alias != self.name_alias:
                        return cmds.aliasAttr(alias, self.combined_name)
                    else:
                        tp.logger.debug('{0}.{1} already has that alias!'.format(self.obj.get_short_name(), self.attr))
                except Exception:
                    tp.logger.warning('{0}.{1} failed to set alias of {2}'.format(self.obj.meta_node, self.attr, alias))
            else:
                if self.name_alias:
                    self.attr = self.long_name
                    cmds.aliasAttr(self.combined_name, remove=True)
        except Exception as e:
            fmt_args = [self.obj.short_name, self.long_name, alias, e]
            tp.logger.error('{0}.{1}.set_alias() | arg: {2} | error: {3}'.format(*fmt_args))
    ...
    def get_nice_name(self):
        ...
        return cmds.attributeQuery(self.attr, node=self.obj.meta_node, niceName=True) or False
    ...
    def get(self, *args, **kwargs):
        ...
        try:
            ...
            tp.logger.warning('{0} failed to get | {1}'.format(self.combined_name, e))
    ...
    def set(self, value, *args, **kwargs):
        ...
        try:
                ...
                if self.attr_type == 'message':
                    ...
                    for i, c in enumerate(self.get_children()):
                        try:
                            ...
                            fmt_args = [c, e]
                            tp.logger.error('On child: {0} | error: {1}'.format(*fmt_args))
                else:
            ...
            tp.logger.error('{0}.{1}.set() | arg: {2} | error: {3}'.format(*fmt_args))
    ...
    def remove(self):
        ...
        try:
            ...
            tp.logger.warning('{} deleted!'.format(self.combined_name))
            del(self)
        except Exception:
            tp.logger.error('{} failed to delete!'.format(self.obj.meta_node, self.attr))
    ...
    def get_locked(self):
        ...
        return cmds.getAttr(self.combined_name, lock=True)
                ...
                if self.get_children():
                    for c in self.get_children():
                        ...
                        if not child_attr.locked:
                            cmds.setAttr(child_attr.obj.meta_node+'.'+child_attr.attr, edit=True, lock=True)
                elif not self.locked:
                    cmds.setAttr(self.obj.meta_node+'.'+self.attr, edit=True, lock=True)
            else:
                if self.get_children():
                    for c in self.get_children():
                        child_attr = MetaAttribute(self.obj.meta_node, c)
                        if child_attr.locked:
                            cmds.setAttr(child_attr.obj.meta_node+'.'+child_attr.attr, edit=True, lock=False)
                elif self.locked:
                    cmds.setAttr(self.obj.meta_node+'.'+self.attr, edit=True, lock=False)
        except Exception as e:
            fmt_args = [self.obj.short_name, self.long_name, lock, e]
            tp.logger.error('{0}.{1}.set_locked() | arg: {2} | error: {3}'.format(*fmt_args))
    ...
    def get_keyable(self):
        ...
        return cmds.getAttr(self.combined_name, keyable=True)
                    ...
                    if self.get_children():
                        for c in self.get_children():
                            ...
                            if not child_attr.keyable:
                                cmds.setAttr(child_attr.obj.meta_node+'.'+child_attr.attr, edit=True, keyable=True)
                                self.hidden = False
                    elif not self.keyale:
                        cmds.setAttr(self.obj.meta_node+'.'+self.attr, edit=True, keyable=True)
                        self.hidden = False
                else:
                    if self.get_children():
                        for c in self.get_children():
                            ...
                            if child_attr.keyable:
                                cmds.setAttr(child_attr.obj.meta_node+'.'+child_attr.attr, edit=True, keyable=False)
                                if not cmds.getAttr(child_attr.combined_name, channelBox=True):
                                    child_attr.set_hidden(False)
                    elif self.keyable:
                        cmds.setAttr(self.obj.meta_node+'.'+self.attr, edit=True, keyable=False)
                        if not cmds.getAttr(self.combined_name, channelBox=True):
                            self.set_hidden(False)
        except Exception as e:
            fmt_args = [self.obj.short_name, self.long_name, keyable, e]
            tp.logger.error('{0}.{1}.set_keyable() | arg: {2} | error: {3}'.format(*fmt_args))
    ...
    def get_hidden(self):
        ...
        hidden = not cmds.getAttr(self.combined_name, channelBox=True)
        if self.keyable:
            hidden = cmds.attributeQuery(self.long_name, node=self.obj.meta_node, hidden=True)
                ...
                if self.get_children():
                    for c in self.get_children():
                        ...
                        if not child_attr.hidden:
                            ...
                            cmds.setAttr(child_attr.obj.meta_node+'.'+child_attr.attr, edit=True, channelBox=False)
                elif not self.hidden:
                    if self.keyable:
                        self.set_keyable(False)
                    cmds.setAttr(self.obj.meta_node+'.'+self.attr, edit=True, channelBox=False)
            else:
                if self.get_children():
                    for c in self.get_children():
                        child_attr = MetaAttribute(self.obj.meta_node, c)
                        if child_attr.hidden:
                            cmds.setAttr(child_attr.obj.meta_node+'.'+child_attr.attr, edit=True, channelBox=True)
                elif self.hidden:
                    cmds.setAttr(self.obj.meta_node+'.'+self.attr, edit=True, channelBox=True)
        except Exception as e:
            fmt_args = [self.obj.short_name, self.long_name, hide, e]
            tp.logger.error('{0}.{1}.set_hidden() | arg: {2} | error: {3}'.format(*fmt_args))
    ...
    def get_default_value(self):
        ...
        try:
            default_value = cmds.attributeQuery(self.long_name, node=self.obj.meta_node, listDefault=True)
            if default_value:
                ...
                if value is not None:
                    if self.get_children():
                        for c in self.get_children():
                            child_attr = MetaAttribute(self.obj.meta_node, c)
                            try:
                                cmds.addAttr(child_attr.obj.meta_node+'.'+child_attr.attr, edit=True, defaultValue=value)
                            except StandardError:
                                tp.logger.debug('"{}" failed to set a default value'.format(child_attr.combined_name))
                    else:
                        try:
                            cmds.addAttr(self.obj.meta_node+'.'+self.attr, edit=True, defaultValue=value)
                        except StandardError:
                            tp.logger.debug('"{}" failed to set a default value'.format(self.combined_name))
        except Exception as e:
            fmt_args = [self.obj.short_name, self.long_name, value, e]
            tp.logger.error('{0}.{1}.set_default() | value: {2} | error: {3}'.format(*fmt_args))
    ...
    def get_min_value(self):
        ...
        try:
            min_value = cmds.attributeQuery(self.long_name, node=self.obj.meta_node, minimum=True)
            if min_value:
                return min_value[0]
            else:
                return False
        ...
        try:
            if self.is_numeric() and not self.get_children():
                if value is False or None:
                    try:
                        cmds.addAttr(self.obj.meta_node+'.'+self.attr, edit=True, hasMinValue=False)
                        tp.logger.warning('{} had its minimum value cleared'.format(self.combined_name))
                    except Exception:
                        tp.logger.error('{} failed to clear a minimum value'.format(self.combined_name))
                elif value is not None:
                    try:
                        cmds.addAttr(self.obj.meta_node+'.'+self.attr, edit=True, minValue=value)
                    except Exception:
                        tp.logger.error('{} failed to set a minimum value'.format(self.combined_name))

                if self.value < value:
                    self.value = value
                    tp.logger.warning('Value changed due to a new minimum. Value is now: {}'.format(value))
            else:
                tp.logger.error('"{}" is not a numeric attribute'.format(self.combined_name))
        except Exception as e:
            fmt_args = [self.obj.short_name, self.long_name, value, e]
            tp.logger.error('{0}.{1}.set_min_value() | value: {2} | error: {3}'.format(*fmt_args))
    ...
    def get_max_value(self):
        ...
        try:
            max_value = cmds.attributeQuery(self.long_name, node=self.obj.meta_node, maximum=True)
            if max_value:
                return max_value[0]
            else:
                return False
    ...
    def set_max_value(self, value):
        ...
        try:
            if self.is_numeric() and not self.get_children():
                if value is False or None:
                    try:
                        cmds.addAttr(self.obj.meta_node+'.'+self.attr, edit=True, hasMaxValue=False)
                        tp.logger.warning('{} had its maximum value cleared'.format(self.combined_name))
                    except Exception:
                        tp.logger.error('{} failed to clear a maximum value'.format(self.combined_name))
                elif value is not None:
                    try:
                        cmds.addAttr(self.obj.meta_node+'.'+self.attr, edit=True, minValue=value)
                    except Exception:
                        tp.logger.error('{} failed to set a maximum value'.format(self.combined_name))

                if self.value > value:
                    self.value = value
                    tp.logger.warning('Value changed due to a new maximum. Value is now: {}'.format(value))
            else:
                tp.logger.error('"{}" is not a numeric attribute'.format(self.combined_name))
        except Exception as e:
            fmt_args = [self.obj.short_name, self.long_name, value, e]
            tp.logger.error('{0}.{1}.set_max_value() | value: {2} | error: {3}'.format(*fmt_args))
    ...
    def get_soft_min_value(self):
        ...
        try:
            min_value = cmds.attributeQuery(self.long_name, node=self.obj.meta_node, softMin=True)
            if min_value:
                return min_value[0]
            else:
                return False
    ...
    def set_soft_min_value(self, value):
        ...
        try:
            if self.is_numeric() and not self.get_children():
                if value is False:
                    try:
                        cmds.addAttr(self.obj.meta_node+'.'+self.attr, edit=True, hasSoftMinValue=False)
                        tp.logger.warning('{} had its minimum value cleared'.format(self.combined_name))
                    except Exception:
                        tp.logger.error('{} failed to clear a soft minimum value'.format(self.combined_name))
                elif value is not None:
                    try:
                        cmds.addAttr(self.obj.meta_node+'.'+self.attr, edit=True, softMinValue=value)
                    except Exception:
                        tp.logger.error('{} failed to set a soft minimum value'.format(self.combined_name))
            else:
                tp.logger.error('"{}" is not a numeric attribute'.format(self.combined_name))
        except Exception as e:
            fmt_args = [self.obj.short_name, self.long_name, value, e]
    ...
    def get_soft_max_value(self):
        ...
        try:
            max_value = cmds.attributeQuery(self.long_name, node=self.obj.meta_node, softMax=True)
            if max_value:
                return max_value[0]
            else:
                return False
    ...
    def set_soft_max_value(self, value):
        ...
        try:
            if self.is_numeric() and not self.get_children():
                if value is False:
                    try:
                        cmds.addAttr(self.obj.meta_node+'.'+self.attr, edit=True, hasSoftMaxValue=False)
                        tp.logger.warning('{} had its maximum value cleared'.format(self.combined_name))
                    except Exception:
                        tp.logger.error('{} failed to clear a soft maximum value'.format(self.combined_name))
                elif value is not None:
                    try:
                        cmds.addAttr(self.obj.meta_node+'.'+self.attr, edit=True, softMaxValue=value)
                    except Exception:
                        tp.logger.error('{} failed to set a soft maximum value'.format(self.combined_name))
            else:
                tp.logger.error('"{}" is not a numeric attribute'.format(self.combined_name))
        except Exception as e:
            fmt_args = [self.obj.short_name, self.long_name, value, e]
    ...
    def get_enum(self):
        ...
        :return: list<str>
        """
        return cmds.attributeQuery(self.attr, node=self.obj.meta_node, listEnum=True)[0].split(':') or False
        ...
        try:
            if self.attr_type == 'enum':
                if ':'.join(self.enum) != metautils.MetaAttributeValidator.string_list_arg(enums):
                    cmds.addAttr(self.obj.meta_node+'.'+self.attr, edit=True, at='enum', en=enums)
                else:
                    tp.logger.info('{} | already set'.format(base_msg))
            else:
                tp.logger.warning('{} | not an enum. Invalid call'.format(base_msg))
        except Exception as e:
            fmt_args = [base_msg, e]
            tp.logger.error('{0} | error: {1}'.format(*fmt_args))
    ...
    def is_multi(self):
        ...
        return cmds.addAttr('{0}.{1}'.format(self.obj.meta_node, self.attr), query=True, m=True)
    ...
    def is_index_matters(self):
        ...
        return cmds.addAttr('{0}.{1}'.format(self.obj.meta_node, self.attr), query=True, im=True)
    ...
    def is_dynamic(self):
        ...
        if self.attr and cmds.listAttr(self.obj.meta_node, userDefined=True):
            return True
        tp.logger.error('{}.is_dynamic: False'.format(self.combined_short_name))
        return False
    ...
    def is_numeric(self):
        ...
        if cmds.getAttr(self.combined_name, type=True) in ['string', 'message', 'enum', 'bool']:
            return False
        return True
    ...
    def is_readable(self):
        ...
        if not self.is_dynamic():
            tp.logger.warning('"{}" is not a dynamic attribute. Readable is not relevant'.format(self.combined_name))
            return False
        return cmds.addAttr(self.combined_name, query=True, r=True) or False
    ...
    def is_writable(self):
        ...
        if not self.is_dynamic():
            tp.logger.warning('"{}" is not a dynamic attribute. Writable is not relevant'.format(self.combined_name))
            return False
        return cmds.addAttr(self.combined_name, query=True, w=True) or False
    ...
    def is_storable(self):
        ...
        if not self.is_dynamic():
            tp.logger.warning('"{}" is not a dynamic attribute. Storable is not relevant'.format(self.combined_name))
            return False
        return cmds.addAttr(self.combined_name, query=True, s=True) or False
    ...
    def is_used_as_color(self):
        ...
        if not self.is_dynamic():
            tp.logger.warning('"{}" is not a dynamic attribute. UsedAsColor is not relevant'.format(self.combined_name))
            return False
        return cmds.addAttr(self.combined_name, query=True, usedAsColor=True) or False
    ...
    def is_user_defined(self):
        ...
        user_defined = cmds.listAttr(self.obj.meta_node, userDefined=True) or []
    ...
    def convert(self, attr_type):
        ...
        try:
            if self.obj.is_referenced():
                tp.logger.error('"{0}" is referenced. Cannot convert "{1}" to "{2}"!'.format(self.obj.meta_node, self.nice_name, attr_type))
            if self.get_children():
                tp.logger.error('"{}" has children, cannot convert'.format(self.combined_name))
            ...
            self.attr_type = cmds.getAttr(self.combined_name, type=True)
        except Exception as e:
            fmt_args = [self.obj.short_name, self.long_name, attr_type, e]
            tp.logger.error('{0}.{1}.convert() | attr_type: {2} | error: {3}'.format(*fmt_args))
    ...
    def get_children(self, as_meta=False):
        ...
        try:
            ...
            try:
                buffer = cmds.attributeQuery(self.attr, node=self.obj.meta_node, listChildren=True) or []
            except Exception:
                buffer = list()
            if as_meta:
            ...
            tp.logger.error('{0} | as_meta: {1} | error: {2}'.format(*fmt_args))
    ...
    def get_parent(self, as_meta=False):
        ...
        try:
            as_meta = metautils.MetaAttributeValidator.bool_arg(as_meta)
            buffer = cmds.attributeQuery(self.attr, node=self.obj.meta_node, listParent=True) or []
            if as_meta:
                return [MetaAttribute(self.obj.meta_node, c) for c in buffer]
            return buffer
        except Exception as e:
